...
static void insertPayloadColumnSpec(llvm::DenseMap<uint64_t, PayloadColumnSpec>& unionCols,
   ...
   for (auto existingIt = unionCols.begin(), e = unionCols.end(); existingIt != e; ++existingIt) {
      ...
      for (unsigned queryIndex : existing.queryIndices)
         if (!llvm::is_contained(spec.queryIndices, queryIndex)) spec.queryIndices.push_back(queryIndex);
      spec.isJoinKey |= existing.isJoinKey;
      spec.stableOrder = existing.stableOrder;
      markSide(spec);
...
static void ingestExternalTableColumnsFromBuildStepScan(subop::ExecutionStepOp buildStep,
   ...
   buildStep.walk([&](subop::ScanRefsOp scanOp) {
      ...
      for (const PayloadColumnSpec& existing : unionSpecs) {
         ...
         spec.isJoinKey = existing.isJoinKey;
         spec.queryIndices = existing.queryIndices;
...
static std::optional<ResidualTableFilter> traceSingleUseStreamToScanLikeInStep(
   ...
   for (;;) {
      ...
      if (auto scan = mlir::dyn_cast<subop::ScanRefsOp>(def)) {
         if (!mlir::isa<subop::TableType, subop::SharedTableType>(scan.getState().getType())) return std::nullopt;
...
static void ensureSyntheticMapHasPeerInputs(subop::ExecutionStepOp syntheticBuild,
   ...
   assert(synthResidual && "residual filter rewrite: synthetic build must have a scan-like source");
   assert(sourceValueMembersForResidualGather(synthResidual->sourceStateType) &&
   ...
   for (unsigned inputIdx = 0; inputIdx < peerMap.getInputCols().size(); ++inputIdx) {
      ...
      subop::Member member = sourceMemberForIdentifier(synthResidual->sourceStateType, mm, leaf);
   ...
   auto gather = b.create<subop::GatherOp>(
      syntheticMap.getLoc(), syntheticMap.getStream(), synthResidual->sourceRef,
...
static void moveSimplePredicateProducerBeforeResidualMap(subop::ExecutionStepOp step,
   ...
   if (predOp == residualMap.getOperation()) return;
   ...
   if (!streamChainContains(streamInputOf(predOp), residualMap.getResult())) return;
...
static std::optional<ResidualTableFilter> findScanLikeSourceInBuildStep(subop::ExecutionStepOp step) {
   ...
   for (mlir::Operation& op : body.without_terminator()) {
      if (auto scan = mlir::dyn_cast<subop::ScanRefsOp>(&op)) {
         if (!mlir::isa<subop::TableType>(scan.getState().getType())) continue;
         auto& cm = scan.getContext()->getLoadedDialect<tuples::TupleStreamDialect>()->getColumnManager();
         return ResidualTableFilter{scan.getOperation(), cm.createRef(&scan.getRef().getColumn()),
...
threadUpstreamMixedPredColumnsToMaterializeStream(subop::ExecutionStepOp buildStep,
   ...
   for (subop::ScanListOp scanList : inputScanLists) {
      auto ler = mlir::dyn_cast<subop::LookupEntryRefType>(scanList.getElem().getColumn().type);
      if (!ler || !mlir::isa<subop::MixedHashIndexedViewType>(ler.getState())) continue;
...
static bool localHivLayoutsSame(subop::HashIndexedViewType a, subop::HashIndexedViewType b) {
   if (!a || !b) return false;
...
static llvm::SmallVector<DirectMixedDep, 4> directMixedDepsForBuildStep(subop::ExecutionStepOp buildStep) {
   ...
   for (mlir::Value input : buildStep.getInputs()) {
      ...
      if (!layout || !mlir::isa<subop::MixedHashIndexedViewType>(input.getType())) continue;
      deps.push_back(DirectMixedDep{*key, layout});
   }
   return deps;
}

static uint64_t directDepKeyForScanList(subop::ScanListOp scanList,
                                        llvm::ArrayRef<DirectMixedDep> directDeps) {
   auto ler = mlir::dyn_cast<subop::LookupEntryRefType>(scanList.getElem().getColumn().type);
   assert(ler && "mixed inherited pred scan_list must expose entry refs");
   auto scanLayout = asLocalHivLayoutType(ler.getState());
   assert(scanLayout && "mixed inherited pred scan_list must scan an HIV-like state");
   std::optional<uint64_t> found;
   for (const DirectMixedDep& dep : directDeps) {
      if (!localHivLayoutsSame(dep.layout, scanLayout)) continue;
      assert(!found && "mixed inherited pred scan_list must match one direct cache dep");
      found = dep.cacheKey;
   }
   assert(found && "mixed inherited pred scan_list must match a direct cache dep");
   return *found;
}
...
threadMappedUpstreamMixedPredColumnsToMaterializeStream(
   ...
   llvm::DenseMap<uint64_t, llvm::DenseSet<unsigned>> wantedSlotsByDep;
   for (const auto& [queryIdx, slot] : itTargetSlots->second) {
      ...
      for (uint64_t depKey : directDepKeys) {
         ...
         wantedSlotsByDep[depKey].insert(itDepSlot->second);
      }
   }
   ...
   for (subop::ScanListOp scanList : inputScanLists) {
      auto ler = mlir::dyn_cast<subop::LookupEntryRefType>(scanList.getElem().getColumn().type);
      if (!ler || !mlir::isa<subop::MixedHashIndexedViewType>(ler.getState())) continue;
      uint64_t depKey = directDepKeyForScanList(scanList, directDeps);
...
static void inheritSlotMapForSyntheticTarget(
   ...
   for (unsigned queryIdx : queries) {
      ...
      if (itSlot == slotBySignature.end()) itSlot = slotBySignature.try_emplace(sig, nextSlot++).first;
...
static bool rewriteSyntheticResidualFiltersAsFilterPreds(subop::ExecutionStepOp syntheticBuild,
   ...
   auto combineWithUpstreamPreds = [&](subop::Member predMember,
      ...
      llvm::SmallVector<tuples::ColumnRefAttr, 4> upstreamRefs =
         threadUpstreamMixedPredColumnsToMaterializeStream(syntheticBuild, mm.getName(predMember));
...
static bool rewriteSyntheticResidualFiltersAsFilterPredsForSlots(
   ...
   auto seedIt = residualBySlot.begin();
   subop::MapOp seedMap = synthResidual ? synthResidual->predMap : seedIt->second.predMap;
   subop::MapOp syntheticMap = synthResidual ? synthResidual->predMap
                                             : createResidualPredicateMapAfterTableScan(syntheticBuild, seedMap,
                                                                                       seedIt->second.filter);
   ...
   for (unsigned qIdx : predQueryIndices) {
      ...
      insertWriteSidePredIntoBufferConstructionStepForPredMember(syntheticBuild, itSimple->second,
                                                                 ...
                                                                 /*allowSharedScanPredicate=*/false);
   ...
   for (unsigned qIdx : predQueryIndices) {
      ...
      if (auto itSimple = simplePredBySlot.find(qIdx); itSimple != simplePredBySlot.end())
         refs.push_back(itSimple->second);
      llvm::SmallVector<tuples::ColumnRefAttr, 4> upstreamRefs =
         threadUpstreamMixedPredColumnsToMaterializeStream(syntheticBuild, mm.getName(predMember));
      refs.append(upstreamRefs.begin(), upstreamRefs.end());
      if (refs.empty()) continue;
      std::string scopeSeed = ("reuse_combined_pred$" + llvm::Twine(qIdx)).str();
      tuples::ColumnRefAttr combinedRef = insertAndPredsBeforeMaterialize(mat, refs, scopeSeed);
      setMaterializeMapping(mat, predMember, combinedRef);
      moveSimplePredicateProducerBeforeResidualMap(syntheticBuild, syntheticMap, combinedRef);
      simplePredBySlot[qIdx] = combinedRef;
   }

   for (auto& it : residualBySlot)
      ensureSyntheticMapHasPeerInputs(syntheticBuild, syntheticMap, it.second.predMap, it.second.filter);
...
static std::string stableTypeKey(mlir::Type type) {
   std::string out;
...
static void alignMaterializedPayloadMembersToUnionPlan(
   ...
   llvm::StringMap<subop::Member> targetMemberBySemanticKey;
   llvm::StringMap<subop::Member> targetMemberByLeafAndType;
   llvm::StringMap<subop::Member> targetMemberByName;
   llvm::StringSet<> ambiguousLeafAndType;
   ...
   for (size_t i = 0; i < plan.payloadColumns.size(); ++i) {
      ...
      targetMemberBySemanticKey[spec.semanticKey] = plan.payloadMembers[i];
      ...
      std::string leafKey =
         (normalizeColumnIdentifier(spec.leaf) + "\x1f" + stableTypeKey(spec.colType)).str();
      if (targetMemberByLeafAndType.contains(leafKey)) {
         targetMemberByLeafAndType.erase(leafKey);
         ambiguousLeafAndType.insert(leafKey);
      } else if (!ambiguousLeafAndType.contains(leafKey)) {
         targetMemberByLeafAndType[leafKey] = plan.payloadMembers[i];
      }
   }
   ...
   for (auto& [member, colRef] : mat.getMapping().getMapping()) {
      ...
      if (member != plan.linkMember && member != plan.hashMember && !isJoinBufferInternalMemberName(memName)) {
         ...
         if (outMember == member) {
            ...
            if (auto itSem = targetMemberBySemanticKey.find(semKey); itSem != targetMemberBySemanticKey.end()) {
               if (!tryUseTarget(itSem->second))
                  llvm_unreachable("join superset: semantic payload member type must match materialized column");
            }
         }
         if (outMember == member) {
            std::string leafKey = (normalizeColumnIdentifier(leaf) + "\x1f" +
                                   stableTypeKey(colRef.getColumn().type)).str();
            if (auto itLeaf = targetMemberByLeafAndType.find(leafKey);
                itLeaf != targetMemberByLeafAndType.end()) {
               (void)tryUseTarget(itLeaf->second);
            }
         }
         if (outMember == member) {
            auto itHash = columnHashes.find(&colRef.getColumn());
            if (itHash != columnHashes.end()) {
               auto it = targetMemberBySemanticHash.find(itHash->second);
...
static void assignPayloadMembersForPlan(subop::MemberManager& mm, lingodb::compiler::dialect::tuples::ColumnManager& cm,
   ...
   llvm::StringMap<subop::Member> byLeafAndType;
   llvm::StringSet<> ambiguousLeafAndType;
   for (auto& [member, colRef] : matOp.getMapping().getMapping()) {
      llvm::StringRef memName = mm.getName(member);
      if (member == plan.linkMember || member == plan.hashMember || isJoinBufferInternalMemberName(memName))
      ...
      if (mm.getType(member) != colRef.getColumn().type) continue;
      auto [scope, leaf] = cm.getName(&colRef.getColumn());
      bySemanticKey.try_emplace(columnSemanticKey(scope, leaf), member);
      std::string leafKey =
         (normalizeColumnIdentifier(leaf) + "\x1f" + stableTypeKey(colRef.getColumn().type)).str();
      if (byLeafAndType.contains(leafKey)) {
         byLeafAndType.erase(leafKey);
         ambiguousLeafAndType.insert(leafKey);
      } else if (!ambiguousLeafAndType.contains(leafKey)) {
         byLeafAndType[leafKey] = member;
      }
   }
   ...
   for (const PayloadColumnSpec& spec : plan.payloadColumns) {
      ...
      if (auto itSem = bySemanticKey.find(spec.semanticKey); itSem != bySemanticKey.end()) {
         if (tryUseMember(itSem->second)) continue;
      }
      std::string leafKey = (normalizeColumnIdentifier(spec.leaf) + "\x1f" + stableTypeKey(slotTy)).str();
      if (auto itLeaf = byLeafAndType.find(leafKey); itLeaf != byLeafAndType.end()) {
         if (tryUseMember(itLeaf->second)) continue;
      }
...
static void syncMaterializeMappingsToBufferMembers(mlir::ModuleOp module, subop::StateMembersAttr targetMembers,
   ...
   module.walk([&](subop::MaterializeOp mat) {
      ...
      for (auto& pr : mat.getMapping().getMapping()) {
         ...
         if (!canon) {
            llvm::SmallVector<subop::Member, 2> candidates;
            for (subop::Member target : targetMembers.getMembers()) {
               if (usedTargetMembers.contains(target)) continue;
               llvm::StringRef targetName = mm.getName(target);
               if (targetName.starts_with("link$") || targetName.starts_with("hash$") ||
                   parseFilterPredMemberSlot(targetName))
                  continue;
               if (mm.getType(target) == pr.second.getColumn().type) candidates.push_back(target);
            }
...
static void syncMaterializeMappingsToActualBufferTypes(mlir::ModuleOp module) {
   ...
   module.walk([&](subop::MaterializeOp mat) {
      ...
      for (auto& pr : mat.getMapping().getMapping()) {
         ...
         if (!bufferMembers.contains(member) || usedMembers.contains(member)) {
            llvm::SmallVector<subop::Member, 2> candidates;
            for (subop::Member target : bufTy.getMembers().getMembers()) {
               if (usedMembers.contains(target)) continue;
               llvm::StringRef targetName = mm.getName(target);
               if (targetName.starts_with("link$") || targetName.starts_with("hash$") ||
                   parseFilterPredMemberSlot(targetName))
                  continue;
...
static void syncGatherMembersToMixedHivLayouts(mlir::ModuleOp module, const llvm::DenseSet<void*>* closureFilter) {
   ...
   auto mixedStateFromGatherRef = [](tuples::ColumnRefAttr ref) -> subop::MixedHashIndexedViewType {
      if (!ref) return nullptr;
      auto ler = mlir::dyn_cast<subop::LookupEntryRefType>(ref.getColumn().type);
      if (!ler) return nullptr;
      return mlir::dyn_cast<subop::MixedHashIndexedViewType>(ler.getState());
   };
   module.walk([&](subop::ExecutionStepOp step) {
      if (closureFilter && !opOperandsOrNestedBlockArgsTouchClosure(step.getOperation(), *closureFilter)) return;

      struct ScopeGatherLayout {
         subop::MixedHashIndexedViewType mixed;
      ...
      for (auto& [scope, layout] : byScope) {
         ...
         for (auto& [oldMember, semantic] : layout.oldMemberSemantic) {
            if (auto it = layout.newMemberBySemantic.find(semantic); it != layout.newMemberBySemantic.end()) {
               oldToNew[oldMember] = it->second;
            }
         }
         for (auto& [oldMember, semantic] : layout.oldMemberSemantic) {
            if (oldToNew.contains(oldMember)) continue;
            mlir::Type oldTy = mm.getType(oldMember);
            subop::Member onlySameType;
            unsigned matches = 0;
            for (subop::Member candidate : layout.newNonPredMembers) {
               if (mm.getType(candidate) != oldTy) continue;
               onlySameType = candidate;
               ++matches;
            }
            if (matches == 1) oldToNew[oldMember] = onlySameType;
         }
      }
...
static void patchBufferBuildStepForUnion(mlir::ModuleOp synthetic, subop::ExecutionStepOp buildStep,
   ...
   if (!scanOp) {
      buildStep.walk([&](subop::ScanRefsOp candidate) {
         if (scanOp) return;
         if (!mlir::isa<subop::TableType>(candidate.getState().getType())) return;
         llvm::StringRef donorTableName;
         ExternalDatasourceProperty ds;
         bool haveDs = false;
         subop::TableType donorTableTy;
         if (!resolveScannedTableExternal(buildStep, candidate.getState(), reuseSynthetic, donorTableName, ds,
                                          haveDs, donorTableTy) ||
             !haveDs)
            return;
         for (const PayloadColumnSpec& spec : plan.payloadColumns) {
            ...
            for (size_t pi = 0; pi < peerHivs.size(); ++pi) {
               ...
               if (columnTypeForIdentifierFromPeerBuildScan(peerBuild, reusePeer, donorTableName, spec.leaf)) {
                  scanOp = candidate;
                  return;
               }
            }
   ...
   auto isPayloadMaterialized = [&](const PayloadColumnSpec& spec) {
      ...
      for (auto& [member, colRef] : matOp.getMapping().getMapping()) {
         ...
         auto [scope, leaf] = cm.getName(&colRef.getColumn());
         if (columnSemanticKey(scope, leaf) == spec.semanticKey) return true;
         if (normalizeColumnIdentifier(leaf) == normalizeColumnIdentifier(spec.leaf) &&
             !isPred) {
            return true;
         }
      }
...
static CachedJoinBufferLayout layoutFromUnionPlan(subop::HashIndexedViewType producerHiv,
   ...
   for (subop::Member producerMember : producerHiv.getValueMembers().getMembers()) {
      ...
      unsigned unionIdx = static_cast<unsigned>(out.payloadMembers.size());
      out.payloadMembers.push_back(producerMember);
      out.payloadColumnTypes.push_back(plan.payloadMemberTypes[planIdx]);
...
static CachedJoinBufferLayout layoutInProducerValueOrder(const CachedJoinBufferLayout& layout,
   ...
   for (auto [producerIdx, producerMember] : llvm::enumerate(producerHiv.getValueMembers().getMembers())) {
      ...
      unsigned newIdx = static_cast<unsigned>(out.payloadMembers.size());
      out.payloadMembers.push_back(producerMember);
      out.payloadColumnTypes.push_back(layout.payloadColumnTypes[oldIdx]);
      llvm::StringRef semantic = layout.payloadSemanticKeys[oldIdx];
      out.payloadSemanticKeys.push_back(std::string(semantic));
...
static CachedJoinBufferLayout layoutFromMaterializeMapping(subop::HashIndexedViewType producerHiv,
   ...
   for (subop::Member member : producerHiv.getValueMembers().getMembers()) {
      ...
      unsigned newIdx = static_cast<unsigned>(out.payloadMembers.size());
      out.payloadMembers.push_back(member);
      out.payloadColumnTypes.push_back(mm.getType(member));
      out.payloadSemanticKeys.push_back(semantic);
      for (unsigned query : knownQueries) {
         if (!queryOwnsSemantic(query, semantic)) continue;
...
static std::optional<tuples::ColumnRefAttr> insertSyntheticAggregateQueryIdFromMixedPreds(
   ...
   buildStep.walk([&](subop::ScanListOp scan) {
      ...
      for (subop::Member member : mixed.getValueMembers().getMembers()) {
         ...
         sourcesBySlot[*slot].push_back(PredSource{scan, member});
      }
   });
   ...
   for (unsigned slot : slots) {
      for (PredSource source : sourcesBySlot[slot]) {
         ...
         predDef.getColumn().type = mlir::IntegerType::get(ctx, 1);
         auto gather = b.create<subop::GatherOp>(
            loc, stream.getType(), stream, cm.createRef(&source.scan.getElem().getColumn()),
...
static void applySyntheticAggregatePayloadUnion(mlir::ModuleOp synthetic, mlir::Value syntheticHtState,
   ...
   if (layout.mixedByQueryId) {
      ...
      filterSlots.push_back(donorQueryId);
      for (const AggregatePeerRewriteInput& peer : peers) {
         filterSlots.push_back(peer.filterSlot);
      }
      ...
      std::optional<tuples::ColumnRefAttr> qidRef =
         insertSyntheticAggregateQueryIdFromMixedPreds(buildStep, filterSlots, donorQueryId);
      if (!qidRef) {
         ...
         qidRef = insertSyntheticAggregateQueryIdColumn(buildStep, clauses, donorQueryId);
      }
   ...
   llvm::StringSet<> producerSemanticTypeKeys;
   unsigned nextSlot = 0;
   ...
   for (const AggregatePeerRewriteInput& peer : peers) {
      ...
      for (unsigned peerIdx = 0; peerIdx < peerInfos.size(); ++peerIdx) {
         ...
         if (producerSemanticTypeKeys.contains(p.semanticKey + "\x1f" + mlirTypeToString(p.type))) continue;
   ...
   auto resolveGatherMember = [&](llvm::StringRef refScope, subop::Member useMem,
      ...
      if (auto byScope = probeMemberSemantic.find(refScope); byScope != probeMemberSemantic.end()) {
         if (auto sem = byScope->second.find(useMem); sem != byScope->second.end()) {
            if (auto it = semKeyToMember.find(sem->second); it != semKeyToMember.end()) return it->second;
         }
      }
...
static bool consumerQueryHadPayloadColumn(const CachedJoinBufferLayout& layout, llvm::StringRef semKey,
   ...
   assert(consumerQueryIndex == 1 && "join layout must record payload ownership for N-way consumers");
            ...
            if (!reused) {
               ...
               if (consumerReuseQueryIndex) {
                  ...
                  if (auto it = layout.querySemanticKeysById.find(*consumerReuseQueryIndex);
                     ...
                     assert(*consumerReuseQueryIndex <= 1 &&
                            "join layout must record payload ownership for N-way consumers");
   ...
   for (const CacheTarget& t : targetsInSynthetic) {
      ...
      if (!unionPredMembers.empty()) {
         ...
         if (predRefs.size() >= 2) {
            ...
            if (!streamIsAlreadyUnionFilteredBySharedTableScan(mat.getStream()))
...
static llvm::SmallVector<unsigned, 8> inheritedMixedPredSlotsInBuildStep(subop::ExecutionStepOp buildStep) {
   ...
   buildStep.walk([&](subop::ScanListOp scanList) {
      ...
      llvm::SmallVector<unsigned, 8> localSlots;
      for (subop::Member member : mixed.getValueMembers().getMembers()) {
      ...
      assert(unionSlot && "mixed HIV scan_list must expose at least one filter_pred slot");
      for (unsigned slot : localSlots) {
         if (slot == *unionSlot) continue;
...
static llvm::SmallVector<uint64_t, 4> inheritedMixedCacheDepsInBuildStep(subop::ExecutionStepOp buildStep) {
   ...
   auto recordCacheGetStep = [&](subop::ExecutionStepOp step) {
      step.walk([&](subop::CacheGetOp get) {
         if (!mlir::isa<subop::MixedHashIndexedViewType>(get.getResult().getType())) return;
...
static JoinBufferUnionPlan buildInheritedMixedPredPlan(mlir::ModuleOp synthetic,
   ...
   subop::BufferType bufTy = getInnerBufferTypeForMaterializeState(mat.getState().getType());
   assert(bufTy && "inherited mixed pred target must materialize into a join buffer");

   subop::CreateHashIndexedView chiv;
   mlir::Value mergedBuffer = resolveJoinMergedBuffer(syntheticHiv, synthetic, reuseSynthetic);
   mlir::Value canonMergedBuffer = canonicalizeStateValueForReuse(mergedBuffer);
   synthetic.walk([&](subop::CreateHashIndexedView op) {
      if (chiv) return;
      if (canonicalizeStateValueForReuse(op.getSource()) == canonMergedBuffer) chiv = op;
   });
   assert(chiv && "inherited mixed pred target must create a hash indexed view");

   JoinBufferUnionPlan plan;
   resolveHashLinkMembersForChiv(chiv, bufTy, mm, plan.hashMember, plan.linkMember);
   assert(plan.hashMember && plan.linkMember && "inherited mixed pred target must have hash/link members");
   ...
   for (auto& [member, colRef] : mat.getMapping().getMapping()) {
      ...
      if (isJoinBufferInternalMemberName(memberName) || parseFilterPredMemberSlot(memberName)) continue;
      auto [scope, leaf] = cm.getName(&colRef.getColumn());
      PayloadColumnSpec spec;
      spec.scope = scope;
      spec.leaf = leaf;
      ...
      spec.semanticHash = payloadColumnIdentityHash(colRef, columnHashes);
      spec.colType = colRef.getColumn().type;
      spec.queryIndices.assign(predSlots.begin(), predSlots.end());
      spec.stableOrder = stableOrder++;
      plan.payloadColumns.push_back(std::move(spec));
   }
   refreshPayloadMemberTypes(plan);
   for (unsigned slot : predSlots) ensureReuseFilterPredColumn(plan, slot, ctx);
   unsigned unionSlot = filterPredUnionSlotForQueryIndices(predSlots);
   ensureFilterPredUnionColumn(plan, unionSlot, ctx);
   return plan;
}
...
void extendSyntheticJoinBuffersWithInheritedMixedPreds(
   ...
   for (const CacheTarget& target : targetsInSynthetic) {
      ...
      if (outLayouts) {
         if (hasPlanLayout)
            planLayout = layoutFromUnionPlan(asHashIndexedViewLayoutType(producer.getType()), plan);
         ...
         if (!previousLayout && hasPlanLayout) previousLayout = &planLayout;
...
void extendSyntheticAggregateHashTablesToPayloadUnionForGroups(
   ...
   for (const CacheTarget& t : targetsInSynthetic) {
      ...
      layout.mixedByQueryId = group.enableFilterPredReuse;
      ModuleReuseInfo reuseSynthetic = collectModuleReuseInfo(synthetic);
      recordAggregateConsumerPayloadLayout(layout, static_cast<unsigned>(donor->query),
                                           queries[donor->query], donor->state,
                                           collectModuleReuseInfo(queries[donor->query]));
      for (const auto& p : collectAggregatePayloadMembers(synthetic, t.state, reuseSynthetic)) {
      ...
      for (const CrossQueryStateMatchEntry& e : group.entries) {
         ...
         peer.filterSlot = aggregateReuseSlotForEntry(e);
         peer.module = queries[e.query];
         peer.state = e.state;
         peer.reuse = collectModuleReuseInfo(peer.module);
         peer.buildStep = findAggregateBuildStepForHt(peer.state, peer.reuse);
         recordAggregateConsumerPayloadLayout(layout, peer.queryId, peer.module, peer.state, peer.reuse);
      ...
      applySyntheticAggregatePayloadUnion(synthetic, t.state, layout, peers, reuseSynthetic,
                                          aggregateReuseSlotForEntry(*donor));
