...
std::optional<HivSourceTableCardinalityEstimate> estimateMergedHivExternalFilterRowsForGroup(
   ...
   for (const CrossQueryStateMatchEntry& entry : group.entries) {
      ...
      if (!target || !mlir::isa<subop::HashIndexedViewType>(target.getType())) return std::nullopt;

      subop::ExecutionStepOp buildStep = findJoinBufferBuildStepForHiv(queries[entry.query], entry.state, reuse);
   ...
   HivSourceTableCardinalityEstimate estimate;
